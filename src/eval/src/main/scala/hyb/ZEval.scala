...
object Z3Eval {
  ...
  // sureState is the portion of the filestate of which we are certain.
  // In other words, it is a conservative mapping from paths to filestates
  // for all files we are certain about.
  def evalR(expr: Expr,
            s0: Z3FileSystemState,
            s1: Z3FileSystemState,
            sureState: State): Z3Bool = {
    evalRHelper(expr.collectPaths, expr, s0, s1, m)
  }
  ...
  def evalRHelper(paths: Set[Path],
                  ...
                  s0: Z3FileSystemState,
                  s1: Z3FileSystemState,
                  sureState: State): Z3Bool = expr match {
    case Error => false
    case Skip => // TODO Figure this out
