...
define users::irc($key, $fullname, $ensure = 'present', $username = $title, $type = 'rsa') {
  ...
  user { "$username":
    ...
    require    => Class['irssi'],
		...
		"""
    val attrs =
      Seq(Attribute(Str("ensure"), Var("ensure"))
        , Attribute(Str("comment"), Var("fullname"))
        , Attribute(Str("managehome"), Bool(true))
        , Attribute(Str("shell"), Str("/usr/bin/irssi")))

    //    Attribute(Str("require"), Res("Class", Str("irssi"), Seq()))
    val edge =
      Edge(E(Res("Class", Str("irssi"), List())),
           E(Res("User", Str("$username"), List())))
       

    //TODO(jcollard): I believe string interpolation needs to happen here
    val dup = Block(Resource(Str("$username"), "user", attrs ), Block(edge, Empty))

    val body = E(ITE(Eq(Str("present"), Var("ensure")),
      Let("dir_ensure", Str("directory"),
        Let("file_ensure", Str("file"),
          Block(Empty, dup))),
      Let("dir_ensure", Str("absent"),
        Let("file_ensure", Str("absent"),
          Block(Empty, dup)))))


    val params =
      Seq(Argument("key", None)
        , Argument("fullname", None)
        , Argument("ensure", Some(Str("present")))
        , Argument("username", Some(Var("title")))
        , Argument("type", Some(Str("rsa"))))

    assert(parse(prog) ==
      Define("users::irc", params, body))
  }
