    ...
    resource match {
      ...
      case ResourceDecl(typ, lst) => {
        val vals = lst.map { case (titleExpr, attrs) => {
          val eas = evalAttrs(st.env, attrs)
          val attrMap = eas.get("stage") match {
            case None => eas + ("stage" -> Str("main"))
            case Some(_) => eas
          }
          ResourceVal(typ, evalTitle(st.env, titleExpr), attrMap)
        }
        }
  ...
  def updateStage(res: (Node, ResourceVal), stages: Map[String, Set[Node]]): Map[String, Set[Node]] = res match {
    case (node, ResourceVal(_, _, attrMap)) =>
      attrMap.get("stage") match {
        ...
        case None => throw EvalError(s"No stage defined in $res.")
      }
  }
