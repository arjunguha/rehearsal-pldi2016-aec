class yoxos(
	...
	#$couchdb_admin_password_uri_escaped = uri_escape($couchdb_admin_password)
        $couchdb_admin_login_uri_escaped = $couchdb_admin_login
	$couchdb_admin_password_uri_escaped = $couchdb_admin_password
	...
	file { "${tomcat6::variables::config_file}.yoxos":
		content => "some content", #(nimish) template('yoxos/tomcat.config.erb'),
		owner => root,
		group => root,
		mode => 0644,
		require => Package['tomcat6'],
	...
	file { "${apache2::variables::config_include_dir}/yoxos.conf":
		...
		content => "some content", #(nimish) template('yoxos/yoxos.httpd.conf.erb'),
		owner => root,
		group => root,
		mode => 0644,
	...
	file { "${data_root}/../slice.properties":
		...
		content => "some content", #(nimish) template('yoxos/slice.properties.erb'),
		owner => root,
		group => root,
		mode => 0644,
