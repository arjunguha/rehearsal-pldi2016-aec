...
class PuppetParse extends RegexParsers
  ...
  lazy val expressions: P[AST] = repsep (expr, ("," | "=>"))
  ...
  lazy val anyparams: P[ResourceParam] = repsep ((param | addparam), ",")


  lazy funcrvalue: P[Function] = 
    NAME <~ "(" ~> expressions.? <~ ")" ^^ {
  ...
  //      with ".?"
  lazy val unless_stmt: P[NotExpr] = 
  ...
  | (expr ~ ("in" | "+" | "-" | "/" | "*" | "%" | "<<" | ">>" | "!=" | "==" | ">" | ">=" | "<" | "<=" | "and" | "or") ~ expr ^^ {
      ...
      case e1 ~ "and" ~ e2 => BoolBinOp (e1, e2, And)
      case e1 ~ "or" ~ e2 =>  BoolBinOp (e1, e2, Or)
    })
  ...
  lazy val caseopt: P[CaseOpt] =
    casevalues <~ ":" ~> "{" ~> stmts_and_decls.? <~ "}" ^^ {
      case csvs ~ None => CaseOpt (csvs, BlockExpr (List (_)))
      case csvs ~ Some (ss) => CaseOpts (csvs, ss)
    }
  ...
  lazy val svalues: P[AST] = selectval | "{" ~> sintvalues <~ ",".? <~ "}"


  lazy val sintvalues: P[ASTArray] = repsep (selectval, ("," | "=>")) ^^ (ASTArray (_))
  ...
  lazy val strings: P[String] = 
    repsep (string, ",") ^^ (_.FoldLeft ("") { case acc, el => acc + el }

  lazy val import: P[Import] = 
    "import" ~> string ^^ (Import (_))
  ...
  lazy val array: P[ASTArray] =
    "[" ~> expressions.? "]" | "[" ~> expression <~ "," <~ "]" ^^ (ASTArray (_))

  lazy val regex: P[ASTRegex] = REGEX ^^ (ASTRegex (_))

  lazy val hash: P[ASTHash] = 
    "{" ~> hashpairs.? <~ "}" | "{" ~> hashpairs <~ ",".? <~ "}" ^^ (ASTHash (_))

  lazy val hashpairs: P[List[(String, Branch)]] = repsep (hashpair, ",")

  lazy val hashpair: P[(String, Branch)] = key <~ "=>" ~> expr ^^ {
    case k ~ e => (k, e)
  }
  ...
  lazy val hasharrayaccesses: P[HashOrArrayAccess] = hasharrayaccess 
